...
if (!stripeSecretKey) {
  console.warn(
    "STRIPE_SECRET_KEY is missing. Checkout will not work until it is set."
  );
}

if (!stripeWebhookSecret) {
  console.warn(
    "STRIPE_WEBHOOK_SECRET is missing. Webhook events will be rejected until it is set."
  );
}
...
const allowedOrigins = new Set(
  (
    ...
    [
      "https://bootybandsfitness.com",
      "https://www.bootybandsfitness.com",
      "https://booty-bands-fitness-2026-7hia.vercel.app",
      "http://localhost:5173",
    ].join(",")
  )
    .split(",")
...
/* =========================================================
   PRODUCT CATALOG
   IMPORTANT:
   Stripe prices are controlled here on the server.
...
const PRODUCT_CATALOG = {
  "starter-kit-fabric-bands": {
    name:
      "Booty Bands Fitness Starter Kit - 3 Non-Slip Fabric Resistance Bands",
    unitAmount: 3999,
  },

  "latex-5-level-band-set": {
    name:
      "Booty Bands Fitness Latex Resistance Bands - 5 Level Workout Set",
    unitAmount: 2999,
  },

  "full-body-training-bundle": {
    name:
      "Booty Bands Fitness Full Body Training Bundle - Resistance Bands With Handles",
    unitAmount: 4999,
  },
};
...
app.get("/health", (req, res) => {
  res.json({
    ...
    corsRestricted: allowedOrigins.size > 0,
  });
});
